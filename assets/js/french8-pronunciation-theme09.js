(() => {
  ...
  const STAGES = [
    {
      label: "Section 1",
      shortLabel: "1",
      audio: "../audio/pronunciation/theme-09/section-1.mp3",
      text: "Le rapport a ete publie hier, mais les donnees devront etre verifiees."
    },
    {
      label: "Section 2",
      shortLabel: "2",
      audio: "../audio/pronunciation/theme-09/section-2.mp3",
      text: "Quand les chercheurs auront compare les sources, ils pourront mettre les points sur les i."
    },
    {
      label: "Section 3",
      shortLabel: "3",
      audio: "../audio/pronunciation/theme-09/section-3.mp3",
      text: "Ce dont nous avons besoin, c'est d'une reformulation claire et nuancee."
    },
    {
      label: "Section 4",
      shortLabel: "4",
      audio: "../audio/pronunciation/theme-09/section-4.mp3",
      text: "En presentant les arguments, les etudiants souhaitant convaincre doivent rester precis."
    },
    {
      label: "Defi final",
      shortLabel: "Defi",
      final: true,
      audio: "../audio/pronunciation/theme-09/n8-09d-synthese-grammaticale-modele-france.mp3",
      text: "Le rapport a ete publie hier, mais les donnees devront etre verifiees. Quand les chercheurs auront compare les sources, ils pourront mettre les points sur les i. Ce dont nous avons besoin, c'est d'une reformulation claire et nuancee. En presentant les arguments, les etudiants souhaitant convaincre doivent rester precis."
    }
  ];

  const API_PATH = "/api/french8/pronunciation-assessment";
  const STORAGE_KEY = "jaralingua:french8:pronunciation-09d:v1";
  const GRADE_SUBMISSION = {
    evaluationId: "pronunciation09d",
    title: "Prononciation 09D - Precision syntaxique B2"
  };
  const LOCAL_URL = "http://127.0.0.1:8020/frances/Niveau%208/ateliers/prononciation-09d-precision-syntaxique.html";
  ...
  resetChallengeButton.innerHTML = '<i class="bi bi-arrow-clockwise"></i> Reinitialiser tout le defi';
  ...
  microphonePicker.innerHTML = '<span><i class="bi bi-mic"></i> Microphone utilise</span><select id="microphoneSelect" aria-label="Choisir le microphone"><option value="">Microphone par defaut</option></select>';
  ...
  function normalizeWord(value) {
    const normalized = value.toLocaleLowerCase("fr-FR").replace(/[\u2019']/g, "'").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z'-]/g, "").replace(/^[-']+|[-']+$/g, "");
    const homophones = {
      "i": "ia",
      "donnee": "donnees",
      "donnes": "donnees",
      ...
      "comprenne": "comprennent",
      "droit": "droits",
      "reviendrai": "reviendrait"
    };
    return homophones[normalized] || normalized;
  }

  function normalizeOralVariants(value) {
    return String(value || "")
      .replace(/\bj[\u2019']sais\b/gi, "jesais")
      .replace(/\bje\s+sais\b/gi, "jesais")
      .replace(/\bchais\b/gi, "jesais")
      .replace(/\bil\s+y\s+a\b/gi, "ya")
      .replace(/\by\s+a\b/gi, "ya")
      .replace(/\by[\u2019']a\b/gi, "ya")
      .replace(/\bt[\u2019']as\b/gi, "tuas")
      .replace(/\btu\s+as\b/gi, "tuas")
      .replace(/\bj[\u2019']te\b/gi, "jete")
      .replace(/\bje\s+te\b/gi, "jete");
  }

  function tokens(value) {
    return normalizeOralVariants(value).split(/\s+/).map(normalizeWord).filter(Boolean);
  }
  ...
  function renderReference(states = []) {
    ...
    readingText.innerHTML = currentStage().text.split(/(\s+)/).map((part) => {
      ...
      return `<button type="button" class="reading-word ${state}" data-word="${index - 1}" data-spoken="${spokenWord(part)}" title="Ecouter ce mot">${part}</button>`;
    }).join("");
  }

  function updateStageUI() {
    const stage = currentStage();
    stageCounter.textContent = stage.final ? "Defi final" : `Pratique guidee · ${currentStageIndex + 1} sur 4`;
    ...
    document.getElementById("resultTitle").textContent = `Resultat · ${stage.label}`;
    modelAudio.pause();
    modelAudio.src = stage.audio;
    modelAudio.load();
    modelButton.querySelector("i").className = "bi bi-play-fill";
    document.querySelector(".player-copy span").textContent = stage.final ? "Modele complet · defi final" : `Modele audio · ${stage.label}`;
    renderHistory();
    renderReference();
  }
  ...
  function pronunciationTip(word) {
    ...
    const special = {
      "rapport": "Gardez le r fran?ais et ne prononcez pas le t final.",
      "publie": "La finale -e reste l?g?re; gardez le participe pass? net.",
      "donnees": "Allongez l?g?rement le groupe donnees et gardez le rythme de la phrase.",
      "verifiees": "Articulez toutes les syllabes : ve-ri-fi-ees.",
      "chercheurs": "Gardez le son ch et le r final doux.",
      "auront": "Ne prononcez pas le t final; liez avec le groupe suivant si n?cessaire.",
      "compare": "La finale -e se prononce clairement, sans ajouter de consonne.",
      "sources": "Gardez le s final muet sauf liaison impos?e par le contexte.",
      "points": "Le t et le s finals restent muets dans points.",
      "dont": "Voyelle nasale : ne prononcez pas le t final.",
      "besoin": "Gardez la voyelle nasale finale, sans ajouter de n audible.",
      "reformulation": "D?coupez le mot en groupes clairs : re-for-mu-la-tion.",
      "claire": "Ouvrez la voyelle et gardez le r doux.",
      "nuancee": "Marquez les trois syllabes : nu-an-cee.",
      "presentant": "Gardez la voyelle nasale finale de presentant.",
      "arguments": "Ne prononcez pas le s final; gardez le groupe r-g fluide.",
      "etudiants": "Le t et le s finals sont muets; gardez la nasalisation.",
      "souhaitant": "Gardez le son oi puis la voyelle nasale finale.",
      "convaincre": "Gardez le son nasal dans con- et terminez par un r doux.",
      "precis": "Le s final reste muet; allongez l?g?rement la derni?re syllabe."
    };
    if (special[lower]) return special[lower];
    if (/(?:er|ez)$/.test(lower)) return "La finale -er ou -ez se prononce /e/, avec une voyelle nette.";
    if (/(?:ais|ait|aient)$/.test(lower)) return "La finale -ais, -ait ou -aient se prononce avec un son ouvert.";
    if (/eau|au/.test(lower)) return "Le groupe eau ou au se prononce /o/ : gardez les levres arrondies.";
    if (/ou/.test(lower)) return "Le groupe ou se prononce /u/, avec les levres bien arrondies.";
    if (/u/.test(lower)) return "Pour le son u, arrondissez les levres et gardez la langue en position de i.";
    if (/(?:an|en|on|in|ain|ein|un)/.test(lower)) return "Voyelle nasale : laissez passer l'air par le nez sans prononcer le n final.";
    if (/oi/.test(lower)) return "Le groupe oi se prononce /wa/, en une seule emission fluide.";
    if (/ch/.test(lower)) return "Le groupe ch se prononce comme dans chat.";
    if (/r/.test(lower)) return "Le r francais se produit doucement au fond de la gorge.";
    if (/(?:ent|s|t|d|x|z)$/.test(lower)) return "Attention a la consonne finale : elle est souvent muette en francais.";
    return "Ecoutez le mot complet, puis repetez-le lentement en gardant le rythme de la phrase.";
  }
  ...
  function feedbackFor(score, missed, wpm) {
    ...
    if (score >= 88) return "Excellente lecture. La section est tres fidele et votre rythme est naturel.";
    if (score >= 70) return `Bonne lecture. Reprenez surtout ${focus || "les mots signales"}.${wpm < 90 ? " Essayez de lire un peu plus continument." : ""}`;
    return `Reecoutez le modele et reprenez cette section par groupes de mots. Travaillez d'abord : ${focus || "les mots en rouge"}.`;
  }
  ...
  function evaluate(transcript, audioDataUrl) {
    ...
    if (!spoken.length) throw new Error("Aucune parole n'a ete reconnue.");
    ...
    if (currentStage().final) {
      ...
      feedback.textContent = `${feedbackFor(overall, missed, wpm)}${guidedAverage === null ? "" : ` Moyenne des sections : ${guidedAverage}/100. Defi final : ${overall}/100.`}${liaisonMessage}`;
      nextButton.innerHTML = '<i class="bi bi-arrow-counterclockwise"></i> Recommencer tout l\'atelier';
    } else {
      const comparison = previousAttempt ? ` ${overall > previousAttempt.overall ? "Vous avez gagne" : overall < previousAttempt.overall ? "Variation" : "Meme resultat"}${overall === previousAttempt.overall ? "" : ` ${Math.abs(overall - previousAttempt.overall)} points`} par rapport a l'essai precedent.` : "";
      feedback.textContent = feedbackFor(overall, missed, wpm) + comparison + liaisonMessage;
      nextButton.innerHTML = currentStageIndex === 3 ? '<i class="bi bi-trophy"></i> Passer au defi final' : '<i class="bi bi-arrow-right"></i> Section suivante';
    }
  ...
  function renderFinalSummary() {
    ...
    finalSummary.innerHTML = `<h3><i class="bi bi-trophy"></i> Bilan final</h3><div><p><span>Meilleure section</span><strong>${STAGES[bestIndex].label} · ${guided[bestIndex].overall}/100</strong></p><p><span>A retravailler</span><strong>${STAGES[needsIndex].label} · ${guided[needsIndex].overall}/100</strong></p><p><span>Evolution</span><strong>${firstAverage} -> ${latestAverage}</strong></p><p><span>Defi final</span><strong>${stageScores[4].overall}/100</strong></p></div>`;
  }
  ...
  async function transcribeAndEvaluate(blob) {
    ...
    try {
      ...
      if (!transcript) {
        if (Number(audioStats.rms || 0) < 0.0008) throw new Error("La grabacion llego en silencio. Seleccione otro microfono y pruebe nuevamente.");
        throw new Error("Whisper recibio sonido, pero no identifico palabras francesas. Hable un poco mas cerca.");
      }
      ...
      recordStatus.textContent = `${currentStage().label} evaluee. Consultez votre resultat.`;
    } catch (error) {
      recordStatus.textContent = "L'analyse n'a pas pu etre terminee.";
  ...
  async function refreshMicrophones() {
    ...
    try {
      ...
      microphoneSelect.innerHTML = '<option value="">Microphone par defaut</option>' + devices.map((device, index) => `<option value="${device.deviceId}">${device.label || `Microphone ${index + 1}`}</option>`).join("");
      if ([...microphoneSelect.options].some((option) => option.value === currentValue)) microphoneSelect.value = currentValue;
    } catch (_error) {
      microphoneSelect.innerHTML = '<option value="">Microphone par defaut</option>';
    }
  }
  ...
  async function start() {
    ...
    try {
      ...
      liveTranscript.textContent = "Lisez uniquement le texte affiche.";
    } catch (error) {
      stopTracks();
      ...
      const messages = {
        NotAllowedError: "L'acces au microphone a ete refuse. Autorisez le microphone pour JaraLingua dans les parametres du navigateur, puis rechargez la page.",
        SecurityError: error?.message === "secure_context" ? "Le microphone exige une connexion HTTPS securisee." : "La politique de securite du navigateur bloque le microphone sur cette page.",
        NotFoundError: "Aucun microphone n'a ete detecte. Verifiez qu'il est connecte et active.",
        NotReadableError: "Le microphone est deja utilise par une autre application. Fermez-la, puis reessayez.",
        AbortError: "L'activation du microphone a ete interrompue. Reessayez.",
        OverconstrainedError: "Le microphone selectionne n'est plus disponible. Choisissez le microphone par defaut.",
        NotSupportedError: "Ce navigateur ne prend pas en charge l'enregistrement audio. Utilisez une version recente de Chrome, Edge ou Safari."
      };
      recordStatus.textContent = messages[error?.name] || "Impossible d'acceder au microphone. Verifiez les autorisations et choisissez un autre peripherique.";
      recordHelp.textContent = "Android ou ordinateur : ouvrez les parametres du site, autorisez le microphone et fermez les applications qui l'utilisent deja.";
      setControls(false, false);
    }
  }
  ...
  function finish() {
    ...
    setControls(false, true);
    recordStatus.textContent = "Preparation de l'analyse...";
    mediaRecorder.stop();
  }
  ...
  function resetAttempt(clearAudio = true) {
    ...
    renderReference();
    liveTranscript.textContent = "Votre transcription apparaitra ici apres l'analyse.";
    recordStatus.textContent = `Pret · ${currentStage().label}`;
  ...
  function confirmResetAllResults() {
    if (!stageScores.some(Boolean) || window.confirm("Reinitialiser tout le defi depuis la section 1 ? Les resultats locaux de cette activite seront effaces.")) {
      resetAllResults();
    }
  }
  ...
  if (location.protocol === "file:") {
    ...
    recordHelp.textContent = "Ouvrez cette activite depuis localhost ou depuis le site HTTPS.";
    unsupported.innerHTML = `<strong>Mode local detecte.</strong><br>Les navigateurs bloquent le microphone sur les adresses file://.<br><a href="${LOCAL_URL}"><i class="bi bi-box-arrow-up-right"></i> Ouvrir la version compatible</a>`;
