(() => {
  ...
  const STAGES = [
    { label: "Section 1 - Final /t/", shortLabel: "1", audio: "audio/unit4/pronunciation/ed-endings/ed-t-contrast.mp3", text: "worked, watched, cooked, washed, laughed, missed." },
    { label: "Section 2 - Final /d/", shortLabel: "2", audio: "audio/unit4/pronunciation/ed-endings/ed-d-contrast.mp3", text: "played, cleaned, lived, called, answered, stayed." },
    { label: "Section 3 - Final /ɪd/", shortLabel: "3", audio: "audio/unit4/pronunciation/ed-endings/ed-id-contrast.mp3", text: "wanted, needed, waited, started, visited, decided." },
    { label: "Final challenge", shortLabel: "Final", final: true, audio: "audio/unit4/pronunciation/ed-endings/final-challenge-regular-ed.mp3", text: "worked, watched, cooked, washed, laughed, missed. played, cleaned, lived, called, answered, stayed. wanted, needed, waited, started, visited, decided." }
  ];

  const GUIDED_COUNT = 3;
  const FINAL_INDEX = STAGES.length - 1;
  const API_PATH = "/api/english-basic/pronunciation-assessment";
  const SUBMIT_PATH = "/api/basic2/unit4-regular-ed-pronunciation/submit";
  const STORAGE_KEY = "jaralingua:english-basic2:pronunciation-unit4-regular-ed:v1";
  const SUBMISSION_KEY = "jaralingua:english-basic2:pronunciation-unit4-regular-ed:submission:v1";
  const LOCAL_URL = "http://127.0.0.1:8020/ingles/basico-2/pronunciation-unit-4-regular-ed-sounds.html";
  ...
  function pronunciationTip(word) {
    ...
    const soundT = new Set(["worked", "watched", "cooked", "washed", "laughed", "missed"]);
    const soundD = new Set(["played", "cleaned", "lived", "called", "answered", "stayed"]);
    const soundId = new Set(["wanted", "needed", "waited", "started", "visited", "decided"]);
    if (soundT.has(lower)) return `${word} ends in /t/. Do not add an extra syllable: work-t, watch-t.`;
    if (soundD.has(lower)) return `${word} ends in /d/. Keep one final voiced sound: play-d, clean-d.`;
    if (soundId.has(lower)) return `${word} ends in /ɪd/. Say an extra syllable clearly: want-ed, need-ed.`;
    return "Listen to the professional model, then repeat the target verb slowly and clearly.";
  }
  ...
  function speakWord(word) {
    ...
    wordAudio = new Audio(`audio/unit4/pronunciation/ed-endings/words/${slug}.mp3`);
    ...
    function update() {
      ...
      if (!report) {
        ...
        if (submitState !== "submitted" && submitState !== "error") {
          setStatus("Complete the 3 sound sections and the final challenge first. Then this button will unlock so you can send the report to the teacher.", "pending");
        }
        return;
      }
      ...
      try {
        const response = await fetch(SUBMIT_PATH, {
          ...
          body: JSON.stringify({
            ...
            activityTitle: "Basic 2 Unit 4 Pronunciation - Regular -ed Sounds",
