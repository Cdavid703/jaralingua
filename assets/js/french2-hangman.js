(function () {
  ...
  var DATA = window.JaraLinguaHangmanA2 || { categories: [] };
  var STORAGE_KEY = "french2-hangman-game-v1";
  var SOUND_KEY = "french2-hangman-sound-v1";
  ...
  function categoryLabel(id) {
    if (id === "all") return "Mélange A2";
    var category = (DATA.categories || []).filter(function (item) { return item.id === id; })[0];
    return category ? category.label : "Catégorie A2";
  }
  ...
  function renderWord(entry) {
    ...
    board.setAttribute("aria-label", state.current.status === "active" ? "Mot à découvrir" : "Solution : " + entry.answer);
  }
  ...
  function init() {
    ...
    if (entries.length !== 100) showMessage("La banque A2 n'a pas chargé toutes ses réponses. Rechargez la page.", "error");
  }
