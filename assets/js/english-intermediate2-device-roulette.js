(() => {
  ...
  const ui = {
    ...
    title: $("rouletteDeviceTitle"),
    model: $("rouletteModel"),
    instruction: $("rouletteInstruction"),
  ...
  function render() {
    ...
    ui.deviceResult.innerHTML = state.device
      ? `<strong>${esc(state.device.label)}</strong><p>Selected device</p>`
      : "<strong>Waiting for a device</strong>";
    ui.list.innerHTML = state.devicePool
      .map(
        (item) =>
          `<button type="button" data-device="${item.id}">${esc(item.label)}</button>`,
      )
      .join("");
    ui.list
    ...
    ui.history.innerHTML = state.rounds.length
      ? state.rounds
          .map(
            (round, index) =>
              `<tr><td>${index + 1}</td><td>${esc(round.student)}</td><td>${esc(round.device)}</td></tr>`,
          )
          .join("")
  ...
  function spin(kind) {
    ...
    state.rotation += 1440 + 360 - (index / pool.length) * 360;
    ...
    window.setTimeout(() => {
      ...
      render();
      setStatus(
        "Selection ready. Complete the spoken turn, then finish the round.",
      );
    }, 2500);
  }
  function selectDevice(device, removed = false) {
    ...
    ui.image.alt = device.label;
    ui.title.textContent = device.label;
    ui.model.textContent = `This is a ${device.label.toLowerCase()}. It is used to ${device.functionText}.`;
    ui.instruction.textContent = `First, ${device.instruction}.`;
    ui.reveal.hidden = false;
    new Audio(data.audioRoot + device.modelAudio).play().catch(() => {});
    render();
  }
  function finish() {
    ...
    state.rounds.push({ student: state.student, device: state.device.label });
    state.student = null;
    state.device = null;
  ...
  $("loadRoster").addEventListener("click", () => {
    state.students = names(ui.roster.value);
    reset();
  });
