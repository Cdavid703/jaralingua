(function () {
  ...
  var API = "/api/french2/imposteur";
  var STATE_API = "/api/french2/imposteur/state";
  var STORAGE_KEY = "french2-imposteur-live-state";
  var SOUND_STORAGE_KEY = "french2-imposteur-sound-enabled";
  var POLL_MS = 1800;
  var QR_VERSION = 3;
  var QR_SIZE = 29;
  ...
  function studentRoomUrl(roomCode) {
    ...
    if (origin) return origin.replace(/\/+$/, "") + "/frances/Niveau%202/jeux/l-imposteur-a2.html?room=" + encodeURIComponent(code) + "#jeu";
  ...
  function populateDeckOptions(room) {
    ...
    if (!options.length && !select.options.length) {
      options = [{ id: "all", label: "Mélange A2", count: 40 }];
    }
    if (options.length) {
  ...
  function messageForError(error) {
    ...
    return {
      ...
      not_enough_players: "Il faut au moins quatre joueurs pour lancer la partie A2.",
  ...
  function waitingRoleMarkup() {
    return '<article class="private-card waiting">' +
      '<img src="../img/jeux/imposteur-a2-role-citoyen.png" alt="Carte privée en attente">' +
  ...
  function renderRole(payload) {
    ...
    if (player.role === "impostor" && room.status !== "waiting") {
      ...
      setHtml("#roleCard", '<article class="private-card impostor">' +
        '<img src="../img/jeux/imposteur-a2-role-imposteur.png" alt="Carte privée imposteur">' +
        '<div><p class="section-kicker">Ton rôle · imposteur</p><h3>' + escapeHtml(impostorCard.term || "Mot proche") + '</h3>' +
        '<p class="category-line">' + escapeHtml(impostorCard.category || room.category || "A2") + '</p><p>' + escapeHtml(impostorCard.brief || "Tu as un mot proche, mais différent du mot de la classe.") + '</p>' +
    ...
    if (player.role === "citizen" && player.card && room.status !== "waiting") {
      ...
      setHtml("#roleCard", '<article class="private-card citizen">' +
        '<img src="../img/jeux/imposteur-a2-role-citoyen.png" alt="Carte privée joueur">' +
  ...
  function renderVote(payload) {
    ...
    setHtml("#votePanel", '<section class="vote-box active"><img src="../img/jeux/imposteur-a2-vote.png" alt="Vote imposteur">' +
