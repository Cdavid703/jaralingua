(function () {
  ...
  var API = "/api/french1/imposteur";
  var STATE_API = "/api/french1/imposteur/state";
  var STORAGE_KEY = "french1-imposteur-live-state";
  var SOUND_STORAGE_KEY = "french1-imposteur-sound-enabled";
  var POLL_MS = 1800;
  var QR_VERSION = 3;
  var QR_SIZE = 29;
  ...
  var ADMIN_EMAILS = ["cdavid.jaramillo@gmail.com", "lcvelasqueza@correo.iue.edu.co", "velasquez.luisac@gmail.com"];
  ...
  function studentRoomUrl(roomCode) {
    ...
    if (origin) return origin.replace(/\/+$/, "") + "/frances/Niveau%201/jeux/l-imposteur-a1.html?room=" + encodeURIComponent(code) + "#jeu";
  ...
  function populateDeckOptions(room) {
    var select = $("#deckSelect");
    if (!select) return;
    var current = (room && room.deck) || select.value || "all";
    var options = room && Array.isArray(room.deckOptions) ? room.deckOptions : [];
    if (!options.length && !select.options.length) {
      options = [{ id: "all", label: "Mélange A1", count: 40 }];
    }
    if (options.length) {
      select.innerHTML = options.map(function (item) {
        return '<option value="' + escapeHtml(item.id) + '">' + escapeHtml(item.label) + ' (' + Number(item.count || 0) + ')</option>';
      }).join("");
    }
  ...
  function messageForError(error) {
    ...
    return {
      ...
      not_enough_players: "Il faut au moins quatre joueurs pour lancer la partie A1.",
  ...
  function waitingRoleMarkup() {
    return '<article class="private-card waiting">' +
      '<img src="../img/jeux/imposteur-a1-role-citoyen.png" alt="Carte privée en attente">' +
  ...
  function renderRole(payload) {
    ...
    if (player.role === "impostor" && room.status !== "waiting") {
      var impostorCard = player.impostorCard || {};
      setHtml("#roleCard", '<article class="private-card impostor">' +
        '<img src="../img/jeux/imposteur-a1-role-imposteur.png" alt="Carte privée imposteur">' +
        '<div><p class="section-kicker">Ton rôle · imposteur</p><h3>' + escapeHtml(impostorCard.term || "Mot proche") + '</h3>' +
        '<p class="category-line">' + escapeHtml(impostorCard.category || room.category || "A1") + '</p><p>' + escapeHtml(impostorCard.brief || "Tu as un mot proche, mais différent du mot de la classe.") + '</p>' +
        '<div class="mini-grid"><div><strong>Pistes possibles</strong><ul><li>Donne une phrase très simple.</li><li>Parle de la catégorie.</li><li>Écoute les détails des autres.</li></ul></div>' +
        '<div><strong>Attention</strong><ul><li>Ton mot n’est pas celui de la classe.</li><li>Ne dis jamais ton mot directement.</li><li>Reste crédible.</li></ul></div></div>' +
    ...
    if (player.role === "citizen" && player.card && room.status !== "waiting") {
      ...
      setHtml("#roleCard", '<article class="private-card citizen">' +
        '<img src="../img/jeux/imposteur-a1-role-citoyen.png" alt="Carte privée joueur">' +
  ...
  function renderVote(payload) {
    ...
    var selectionNote = selectedSuspectId
      ? '<p class="ready-note vote-choice-note" id="voteChoiceNote"><i class="bi bi-person-check"></i> Sélection : ' + escapeHtml(selectedName) + '. Envoie ou modifie ton vote avant la révélation.</p>'
      : '<p class="muted-line vote-choice-note" id="voteChoiceNote">Aucun suspect sélectionné. Choisis un camarade avant d’envoyer.</p>';
    var savedNote = player.hasVoted
      ? '<p class="ready-note"><i class="bi bi-check2-circle"></i> Ton vote est enregistré. Tu peux encore le modifier avant la révélation.</p>'
      : "";
    var buttonLabel = player.hasVoted ? "Modifier mon vote" : "Envoyer mon vote";
    setHtml("#votePanel", '<section class="vote-box active"><img src="../img/jeux/imposteur-a1-vote.png" alt="Vote imposteur">' +
      '<div><p class="section-kicker">Vote secret</p><h3>Qui est l\'imposteur ?</h3>' +
      '<p>Choisis la personne qui semblait ne pas connaître le mot secret.</p>' + savedNote +
  ...
  function handleVoteSelection(input) {
    ...
    if (note) {
      ...
      note.innerHTML = '<i class="bi bi-person-check"></i> Sélection : ' + escapeHtml(playerNameById((lastPayload && lastPayload.players) || [], input.value)) + '. Envoie ou modifie ton vote avant la révélation.';
    }
  }
  ...
  function renderResult(payload) {
    ...
    setHtml("#resultPanel", '<section class="result-box-final"><p class="section-kicker">Révélation</p><h3>Imposteur : ' + impostors + '</h3>' +
      '<p>Mot de la classe : <strong>' + escapeHtml(result.card && result.card.term) + '</strong><br>Mot de l’imposteur : <strong>' + escapeHtml(result.card && result.card.impostorTerm) + '</strong></p><ul class="vote-summary">' + votes + '</ul></section>');
  }
  ...
  async function handleCreateRoom() {
    ...
    try {
      var result = await request("create", { deck: ($("#deckSelect") && $("#deckSelect").value) || "all" });
  ...
  async function teacherAction(action) {
    ...
    try {
      var payload = { roomCode: localState.roomCode, teacherToken: localState.teacherToken };
      if (action === "distribute" || action === "reset") payload.deck = ($("#deckSelect") && $("#deckSelect").value) || "all";
  ...
  async function submitVote(event) {
    ...
    try {
      ...
      lastPayload = result.state;
      render(lastPayload);
      showMessage("Vote enregistré. Tu peux encore le modifier avant la révélation.", "success");
