(function () {
  ...
  var DATA = window.JaraLinguaHangmanA1 || { categories: [] };
  var STORAGE_KEY = "french1-hangman-game-v1";
  var SOUND_KEY = "french1-hangman-sound-v1";
  ...
  function categoryLabel(id) {
    if (id === "all") return "Mélange A1";
    var category = (DATA.categories || []).filter(function (item) { return item.id === id; })[0];
    return category ? category.label : "Catégorie A1";
  }
  ...
  function init() {
    ...
    if (entries.length !== 100) showMessage("La banque A1 n'a pas chargé toutes ses réponses. Rechargez la page.", "error");
  }
