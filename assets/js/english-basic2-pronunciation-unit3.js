(() => {
  ...
  const STAGES = [
    { label: "Section 1 - Place adjectives", shortLabel: "1", audio: "audio/unit3/pronunciation/section-1-place-adjectives.mp3", text: "ancient, historic, modern, traditional." },
    { label: "Section 2 - Atmosphere adjectives", shortLabel: "2", audio: "audio/unit3/pronunciation/section-2-atmosphere-adjectives.mp3", text: "crowded, quiet, peaceful, remote." },
    { label: "Section 3 - Character adjectives", shortLabel: "3", audio: "audio/unit3/pronunciation/section-3-character-adjectives.mp3", text: "scenic, spacious, touristy, impressive." },
    { label: "Section 4 - Comparatives", shortLabel: "4", audio: "audio/unit3/pronunciation/section-4-comparatives.mp3", text: "Lake Miran is quieter than Valeria. Valeria is more crowded at noon. The hotel is more spacious than the hostel." },
    { label: "Section 5 - Superlatives", shortLabel: "5", audio: "audio/unit3/pronunciation/section-5-superlatives.mp3", text: "The tower is the most impressive landmark. Lake Miran is one of the quietest places in the guide. The main square is the most crowded place at noon." },
    { label: "Section 6 - Unit 3 descriptions", shortLabel: "6", audio: "audio/unit3/pronunciation/section-6-unit-3-descriptions.mp3", text: "Lake Miran is peaceful and scenic. Valeria is historic and touristy. The lake village is more remote, but the city has an impressive museum." },
    { label: "Final challenge", shortLabel: "Final", final: true, audio: "audio/unit3/pronunciation/final-challenge-around-the-world.mp3", text: "ancient, historic, modern, traditional. crowded, quiet, peaceful, remote. scenic, spacious, touristy, impressive. Lake Miran is quieter than Valeria. Valeria is more crowded at noon. The hotel is more spacious than the hostel. The tower is the most impressive landmark. Lake Miran is one of the quietest places in the guide. The main square is the most crowded place at noon. Lake Miran is peaceful and scenic. Valeria is historic and touristy. The lake village is more remote, but the city has an impressive museum." }
  ];

  const GUIDED_COUNT = 6;
  const FINAL_INDEX = STAGES.length - 1;
  const API_PATH = "/api/english-basic/pronunciation-assessment";
  const SUBMIT_PATH = "/api/basic2/unit3-pronunciation-around-world/submit";
  const STORAGE_KEY = "jaralingua:english-basic2:pronunciation-unit3-around-world:v1";
  const SUBMISSION_KEY = "jaralingua:english-basic2:pronunciation-unit3-around-world:submission:v1";
  const LOCAL_URL = "http://127.0.0.1:8020/ingles/basico-2/pronunciation-unit-3-around-world.html";
  ...
  function pronunciationTip(word) {
    ...
    const special = {
      "ancient": "Stress the first syllable: AN-cient. The middle sound is /sh/.",
      "historic": "Keep the initial h and stress the second syllable: his-TOR-ic.",
      "modern": "Stress the first syllable: MOD-ern.",
      "traditional": "Stress the second syllable: tra-DI-tion-al.",
      "crowded": "Say two clear syllables: CROW-ded.",
      "quiet": "Say two syllables: QUI-et.",
      "peaceful": "Start with the long ee sound: PEACE-ful.",
      "remote": "Stress the second syllable: re-MOTE.",
      "scenic": "Use the long ee sound: SCE-nic.",
      "spacious": "Stress the first syllable: SPA-cious. Keep the /sh/ sound.",
      "touristy": "Start with TOUR and keep four clear syllables: TOUR-is-ty.",
      "impressive": "Stress the second syllable: im-PRES-sive.",
      "quieter": "Say three syllables: QUI-et-er.",
      "quietest": "Say three syllables: QUI-et-est.",
      "valeria": "Keep four syllables clear: va-LE-ri-a.",
      "miran": "Say two smooth syllables: mi-RAN.",
      "landmark": "Stress LAND and keep the final k sound.",
      "square": "Start with the /skw/ sound and hold the vowel.",
      "village": "Say two syllables: VIL-lage.",
      "museum": "Stress the second syllable: mu-SE-um."
    };
    if (special[lower]) return special[lower];
    if (/th/.test(lower)) return "Place the tongue lightly between the teeth and let the air pass for th.";
    ...
    if (/s$/.test(lower)) return "Keep the final s sound short and clear.";
    return "Listen to the complete word, then repeat it slowly with the same stress and number of syllables.";
  }

  function showWordHelp(word, playing = false) {
    if (!word) return;
    wordHelp.hidden = false;
    wordHelp.innerHTML = `<strong><i class="bi ${playing ? "bi-volume-up" : "bi-info-circle"}"></i> ${word}</strong><span>${pronunciationTip(word)} ${playing ? "Playing the individual ElevenLabs word model now." : "Every visible word has its own ElevenLabs audio model."}</span>`;
  }

  let wordAudio = null;
  function wordAudioSlug(value) {
    return normalizeWord(value).replace(/'/g, "");
  }

  function speakWord(word) {
    const cleanWord = spokenWord(word || "");
    const slug = wordAudioSlug(cleanWord);
    if (!slug) return;
    modelAudio.pause();
    modelButton.querySelector("i").className = "bi bi-play-fill";
    if (wordAudio) {
    ...
    wordAudio = new Audio(`audio/unit3/pronunciation/words/${slug}.mp3`);
    wordAudio.preload = "auto";
    wordAudio.playbackRate = 0.9;
    let failed = false;
    wordAudio.addEventListener("error", () => {
      failed = true;
      showWordHelp(cleanWord);
      wordHelp.querySelector("span").textContent = "This professional word audio is unavailable. Please report this word to your teacher.";
    }, { once: true });
    wordAudio.play().then(() => {
      if (!failed) showWordHelp(cleanWord, true);
    }).catch(() => {
      if (!failed) {
        showWordHelp(cleanWord);
        wordHelp.querySelector("span").textContent = "Tap the word again to play its professional ElevenLabs model.";
      }
    });
  }
