...
La dinámica usa dos ruletas independientes. La primera selecciona un estudiante y la segunda selecciona un único prompt; ambos elementos se retiran temporalmente de su conjunto para garantizar turnos variados y justos. La docente cierra la ronda únicamente después de una respuesta oral de dos o tres ideas conectadas y una pregunta corta de seguimiento de un compañero —por ejemplo, `Why?` o `What would happen next?`—. El historial visible funciona como apoyo de conducción de la clase, no como registro evaluativo; Reset vuelve a habilitar todos los nombres y preguntas.

Hay exactamente veinte preguntas: diez comienzan con **If** y diez comienzan con **Would**. Todas sitúan al estudiante en un presente o futuro imaginado y piden una respuesta abierta, defendible y segura. Cada pregunta elegida muestra un frame lingüístico de apoyo sin imponer un guion: `If I had…, I would…`, `I would / would not… if…`, etc. Antes de las ruletas, la página explica explícitamente que el second conditional expresa distancia de la realidad, no tiempo pasado, y presenta los dos órdenes equivalentes: `If I had a free year, I would travel.` / `I would travel if I had a free year.`
