...
La actividad convierte la lengua de Unit 2 en una interacción real: el grupo selecciona uno de cuatro dilemas seguros y no invasivos —curso o trabajo, mudarse o quedarse, ahorrar o crear, segunda oportunidad— y puede mantenerlo ficticio. La tarjeta no se limita a marcar una selección: despliega debajo una ficha específica del caso con contexto, tres opciones defendibles, una pregunta distinta para Moderator, Dreamer, Realist y Adviser, una consecuencia que se debe pesar y el modelo de conclusión oral. Cada estudiante adopta un papel rotativo; el diseño prevé además una adaptación explícita para grupos de tres o cinco, por lo que nadie queda sin turno.
...
## 52. Implementación confirmada — Unit 2 Conversation Coach: A New Friend, A Real Choice

`conversation-coach-unit-2-renata-real-choice.html` es una actividad de la Unidad 2 y vive exclusivamente en Practice Lab. Es una conversación privada, formativa y no evaluativa: no muestra porcentaje, no crea nota, no se proyecta al gradebook y no se envía al profesor. La grabación y la transcripción se usan únicamente durante la sesión en pantalla; el estudiante puede regrabar, continuar sin transcripción o reiniciar la conversación.

La ruta no comienza de forma artificial con un dilema. Renata y el estudiante todavía no se conocen, por lo que sus siete turnos avanzan de manera natural: (1) saludo y nombre; (2) conexión sencilla sobre lo que hacen después de clase; (3) una meta futura con `hope to`, `would like to` o `goal is to`; (4) una pregunta completa y específica sobre un curso de diseño; (5) una consecuencia posible de que ese curso coincida con un compromiso familiar; (6) un consejo práctico con `could` o `If I were you, I would`; y (7) una pregunta amable para cerrar. Así se entrena conversación real antes de trabajar wishes, advice and regrets de forma explícita.

Cada turno ofrece modelos opcionales, vocabulario y un foco claro, sin convertir la conversación en un guion obligatorio. El último turno reconoce preguntas sobre fecha/curso, familia/proyecto o esperanza/plan, y Renata responde con un audio diferente y pertinente; una respuesta general también recibe una reacción clara. La aplicación comparte el motor maduro de Conversation Coach de Unidad 1, ahora configurable por personaje, ruta, respuestas finales y carpeta de audio. Esa reutilización conserva la comprobación de micrófono, reproductor de la respuesta propia, transcripción temporal, recuperación ante fallos y recapitulación privada, sin introducir `speechSynthesis`.

La actividad incorpora una imagen profesional propia de dos jóvenes conversando tras clase, sin texto, logo, watermark ni blackboard. La interfaz sigue la norma permanente de expansión compacta: hero único, buscador, ruta, grabación y banco de lenguaje usan el ancho disponible de móvil, tablet, portátil y escritorio; los grupos se reorganizan y se apilan antes de comprimir texto o controles.

Todos los 22 audios audibles son MP3 reales de ElevenLabs, generados desde `audio/conversation-coach/unit-2-renata-real-choice/scripts.md` con Sarah — Clear US (`EXAVITQu4vr4xnSDxMaL`): bienvenida, instrucciones, siete preguntas, seis reacciones, cuatro respuestas finales, cierre y dos recuperaciones. `metadata.json` registra el proveedor, voz, fecha y cantidad. Ningún botón de audio depende de la voz del navegador.
...
El profesor presenta una tarjeta visual rica en detalles, divide al grupo en dos posiciones —For y Against— y activa un cronómetro visible de cinco minutos para cada situación. Durante esos 300 segundos, todos los estudiantes permanecen sentados y cada uno prepara exactamente una recomendación de acuerdo con su lado. No se exige explicar razones, consecuencias ni argumentos extensos. Al terminar el tiempo suena una alarma; el profesor pregunta primero `Who would like to start?` y, si no hay voluntarios, llama a los estudiantes en orden. Durante la conversación puede introducir preguntas breves para mantenerla activa.

La actividad contiene cinco dilemas visuales: aceptar una beca o quedarse para apoyar a la familia; abrir una panadería o conservar la estabilidad laboral; reportar o proteger a una amistad que hizo trampa; aceptar un ascenso que implica mudar a la familia; y realizar una gira musical o terminar la carrera. Son situaciones ficticias, seguras y apropiadas para el nivel, presentadas sin trauma explícito ni moralización. Cada tarjeta ofrece una imagen diciente, contexto, pregunta central y una instrucción diferenciada para For y Against.

Las respuestas practican las estructuras de consejo de Unit 2: `should / shouldn't + verb`, `could + verb`, `might want to + verb`, `If I were ..., I would ...`, `Have you considered + -ing?` e `I suggest + -ing`. La interfaz recuerda expresamente que basta una recomendación clara. Incluye un modelo completo profesor–estudiante para ambos lados, preguntas opcionales del profesor y una lista de comprobación posterior que confirma que hablaron ambas posiciones, participó cada estudiante y se usó una estructura de la unidad.

La secuencia pedagógica está visible como Pre-task, Task y Post-task. Antes de la práctica aparecen el objetivo, el foco lingüístico y el producto oral; después del hero único aparece inmediatamente un buscador que filtra los cinco dilemas. El selector usa tarjetas horizontales compactas con miniaturas propias, número y título. Al cambiar de situación se reinician el cronómetro y el checklist. Mientras el tiempo corre se bloquean el buscador y la navegación para conservar la ronda activa; al llegar a cero se muestra una alarma visual y audible creada con Web Audio, desbloqueada por la acción del profesor y sin `speechSynthesis`.

El diseño toma como referencia funcional las actividades Decision Room y Family Conflict Mediation del curso anterior, pero aplica de forma completa el sistema visual de Intermediate English Course 2: navy `#071f4f`, blue `#2454a6`, teal `#0f766e`, coral `#d9433f`, gold `#d9a441`, fondo `#f4f7fb`, paper `#fff` y accent `#5b4bc4`. Hero, búsqueda, meta pedagógica, instrucciones, modelo y consola usan `width: 100%` y `max-width: none`, con gutters seguros mediante `env(safe-area-inset-*)`, sin `100vw` ni contenedor central estrecho. Las cuadrículas se reducen progresivamente para teléfono, tablet, portátil y escritorio; en móvil las tarjetas del selector conservan miniaturas de 96 px y no generan desplazamiento horizontal.

La imagen de hero es exclusiva de la actividad y muestra una conversación presencial con dos lados y mediación docente. Las cinco escenas y el hero son WebP sin texto incrustado, logos, marcas de agua ni estética blackboard; la imagen principal no se repite inmediatamente bajo el banner. El cierre incluye navegación de regreso a la carpeta de Unit 2 en Practice Lab y a la teoría de advice.

La entrada `unit-2-take-a-side-visual-dilemmas` del catálogo registra cinco dilemas, `preparationSeconds: 300`, orden 6, estado publicado y el hero exclusivo. El contrato estático `tools/test_intermediate2_take_a_side_page.mjs` verifica estructura pedagógica, recursos visuales, búsqueda, miniaturas, temporizador, alarma, lenguaje de Unit 2, accesibilidad básica, integración de catálogo, diseño expandido y documentación. El cierre obligatorio mantiene la regla del curso: pruebas estáticas sin servidor local, commit, push, despliegue selectivo y comprobación directa en producción.

## 56. Implementación confirmada — Unit 2 Speaking Game: Reality or Imagine?

`speaking-unit-2-reality-or-imagine-roulette.html` pertenece exclusivamente a Practice Lab, Unidad 2, como actividad 08 de conversación oral dirigida por el profesor. La página se proyecta para el grupo y no es una tarea individual: no graba, no solicita entrega, no muestra porcentaje, no crea nota y no altera Grades.

La dinámica usa dos ruletas independientes. La primera selecciona un estudiante y la segunda selecciona el inicio incompleto de un pensamiento; ambos elementos se retiran temporalmente de su conjunto para garantizar turnos variados y justos. La docente cierra la ronda únicamente después de que el estudiante complete oralmente la otra mitad de la estructura y un compañero formule una pregunta corta de seguimiento —por ejemplo, `Why?` o `What would happen next?`—. El historial visible funciona como apoyo de conducción de la clase, no como registro evaluativo; Reset vuelve a habilitar todos los nombres y starters.

Hay exactamente veinte starters: diez comienzan con **If** y diez comienzan con **Would**. Todas sitúan al estudiante en un presente o futuro imaginado y piden una continuación abierta, defendible y segura. Un starter que comienza con `If` debe completarse con `would + base verb` —por ejemplo, `If accepting a scholarship meant leaving your family or friends, I would…`—. Un starter que comienza con `Would` debe completarse con una cláusula `if + past` —por ejemplo, `Would you accept a scholarship if it meant leaving family or friends?`. Antes de las ruletas, la página explica explícitamente que el second conditional expresa distancia de la realidad, no tiempo pasado, y presenta los dos órdenes equivalentes: `If I had a free year, I would travel.` / `I would travel if I had a free year.`

El botón **Load Intermediate 2 roster** usa la sesión autenticada de docente o administrador para pedir únicamente los nombres de la grilla de Intermediate English Course 2. El servidor ya protege esa lectura: el endpoint de notas responde la lista completa solo a `teacher` o `admin`; alumnos o sesiones inexistentes no reciben la lista. La herramienta no muestra, guarda ni exporta email, documento, contacto, calificaciones o trabajo del alumnado. Como respaldo útil para clase, la profesora puede pegar una lista o importar Excel/CSV/TSV/TXT y los nombres quedan solamente en la memoria de la página actual.

### 56.1 Regla de composición de la grilla para la ruleta

La grilla de **Intermediate English Course 2** conserva sus estudiantes inscritos y agrega a quienes aprobaron **Intermediate English Course 1**. Para esta promoción, “aprobó” significa que tiene una nota numérica en las cinco evaluaciones oficiales ponderadas de Intermedio 1 y que su promedio ponderado es como mínimo `3.0`. Solo se copian datos de identidad necesarios para la cuenta del nuevo curso —ID, nombre, correo/alias y contacto cuando existe—; nunca se trasladan calificaciones, entregas, detalles de evaluación ni historial de Intermedio 1. La carga de la ruleta recibe exclusivamente `fullName` de esa grilla protegida.

### 56.2 Estándar obligatorio de construcción de actividades — no inventar la arquitectura

Desde este momento, **ninguna actividad de Intermediate English Course 2 se construye como una página aislada o un diseño nuevo improvisado**. Antes de escribir HTML, CSS o JavaScript se debe mapear una actividad equivalente ya publicada en Basic English Course 1, Basic English Course 2 o Intermediate English Course 1 y registrar qué elementos estructurales se reutilizan. La referencia debe elegirse por tipo de experiencia —reading, listening, pronunciation, grammar, game/roulette, group speaking o Conversation Coach—, no solo porque use colores similares.

#### Arquitectura visual que toda actividad debe respetar

1. **Navegación del curso:** switcher global y navegación de Intermediate English Course 2, con enlaces de retorno a Practice Lab y a la explicación exacta de la unidad. No se reemplaza por una cabecera inventada.
2. **Hero/banner inmersivo obligatorio:** a ancho completo, fondo navy institucional `#092453`, copy blanco a la izquierda, kicker dorado, título claro, una sola descripción útil, chips de metadatos y acciones. La imagen profesional exclusiva ocupa el bloque derecho en escritorio. Este patrón se toma de actividades como *The Better Choice Roundtable*, *Take a Side*, listening, reading y grammar de Intermediate English Course 2.
3. **Hero responsive:** en tablet y móvil el copy pasa antes de la imagen; la imagen se ubica debajo y se limita aproximadamente a `220–280px` de alto en móvil. El hero nunca queda fijo, no tapa el contenido y no se convierte en una tarjeta blanca dividida sin identidad de curso.
4. **Imagen única y pertinente:** hero con imagen profesional relacionada con el objetivo, sin letras incrustadas, logos, watermark o blackboard. Esa misma imagen no vuelve a aparecer inmediatamente debajo del banner. Las imágenes secundarias solo se añaden cuando realmente guían una decisión, historia o interacción.
5. **Inicio de contenido compacto:** después del hero se usa buscador cuando la página contiene rutas, materiales o varios elementos localizables; luego aparece una franja o bloque breve con objetivo, foco de lenguaje y producto oral/escrito. No se repite el título de unidad, la descripción del hero ni la imagen antes de enseñar o activar la actividad.
6. **Cuerpo expandido y compacto:** los contenedores usan todo el ancho disponible con gutters seguros, `minmax(0, …)`, sin `100vw` ni columnas artificialmente estrechas. En escritorio se combinan contenidos relacionados; en tablet se reorganizan; en móvil se apilan antes de reducir texto, imágenes o controles. Toda página se revisa al menos para 360–390px, móvil amplio, tablet, portátil y escritorio.
7. **Interacción fiel al tipo de actividad:** una ruleta incluye configuración de nombres, giro visible, selección clara, historial y la misma retroalimentación sonora de las ruletas de referencia; un listening incluye audio real de ElevenLabs y sus fases; pronunciación conserva sus secciones, clics de palabra, grabación y envío ya definidos; Conversation Coach conserva su ruta, micrófono y reacciones; reading y grammar siguen sus comprobaciones. No se sustituyen estos flujos por tarjetas genéricas o instrucciones sobre “cómo se enseñaría”.
8. **Evaluación y privacidad:** prácticas no oficiales no muestran porcentaje, calificación, “0%”, entrega docente ni Grades. Si la actividad usa una grilla, se piden solamente los nombres imprescindibles con acceso docente protegido; no se exponen correos, identificaciones, notas ni entregas.
9. **Audio:** toda voz, explicación, modelo o botón pedagógico audible usa un archivo real de ElevenLabs. Los efectos mínimos de interfaz —por ejemplo, ticks de una ruleta o alarma de temporizador— pueden usar Web Audio únicamente después de una acción explícita del docente, nunca `speechSynthesis` ni reproducción automática. **La plataforma de producción no es parte de la experiencia pública:** “ElevenLabs” no se muestra en texto, chips, botones, etiquetas `aria`, descripciones SEO, buscadores, tarjetas ni resúmenes de catálogo para estudiantes o docentes. El proveedor se conserva solamente en metadatos, guiones, scripts y registros técnicos internos.

#### Control obligatorio antes de aprobar una actividad

Antes de declarar una actividad lista se debe comprobar y dejar constancia de que: (a) se revisó y nombró la actividad modelo de los niveles anteriores; (b) el hero sigue el patrón navy + copy + imagen y no duplica contenido; (c) la actividad está únicamente en Practice Lab y Course Overview solo enlaza a teoría; (d) la imagen y todo audio requerido existen; (e) no hay porcentajes o entregas impropias; (f) el catálogo, buscador y contador se actualizan; (g) se ejecuta una prueba estática sin servidor local; y (h) se hace commit, push, despliegue selectivo y comprobación en producción.

Este estándar es permanente para las actividades ya creadas y para todas las siguientes. Si una actividad anterior se detecta con hero, arquitectura o responsividad diferente, se debe adaptar a este estándar antes de continuar con nuevas páginas.

Su hero es una imagen profesional propia de clase presencial, sin texto, logo, watermark ni estética blackboard; no se vuelve a repetir bajo el banner. Debe usar el mismo patrón de banner inmersivo de las actividades de Intermedio 2: bloque azul marino a ancho completo, copy blanco y kicker dorado a la izquierda, imagen grande ocupando la mitad derecha, chips translúcidos y acciones coherentes sobre el banner. En tablet y móvil el copy se presenta primero y la imagen pasa debajo, con 220–280 px de alto; no se mantiene un hero blanco dividido. El diseño usa el ancho disponible con gutters seguros y contiene configuración, dos ruletas, resultados, pool e historial en una estructura compacta: dos columnas donde existe espacio y una columna amplia, botones completos y controles alcanzables en móvil. No emplea `100vw`, barras fijas ni contenido que desborde horizontalmente. Cada giro activa, exclusivamente tras el clic de la docente, el mismo efecto breve de ticks y confirmación usado por las ruletas de referencia: es un efecto de interfaz con Web Audio, no una voz, explicación o material pedagógico. No usa `speechSynthesis`, no se inicia automáticamente y no sustituye ningún audio de ElevenLabs.

El catálogo registra `unit-2-reality-or-imagine-roulette`, orden 8, tipo `speaking-game`, 20 starters repartidos `if: 10` y `would: 10`, duración 15–25 minutos, hero exclusivo y carga protegida de roster docente. Todo cambio futuro de esta actividad debe conservar este contrato y el cierre estándar: prueba estática sin servidor local, commit, push, despliegue selectivo y comprobación directa en producción.
## 57. Entregas formativas independientes y activación de Grades

`The Call Before Midnight` y `The Six-Week Window` son actividades formativas entregables al docente desde Practice Lab. El estudiante debe completar y revisar las diez respuestas antes de habilitar **Send to teacher**. Cada actividad conserva un buzón independiente, identidad autenticada, fecha, respuestas completas y comprobante idempotente `JLF-`.

Ninguna de las dos actividades crea nota, fila, porcentaje, `gradeDetails` o proyección en Intermediate English Grades. El catálogo usa `teacherSubmission: true`, `gradebookProjected: false`, `gradebookWeight: null` y `affectsAverage: false`; la interfaz no muestra `0%`. Los buzones son visibles al personal docente y cada estudiante solo puede recuperar sus propios comprobantes.

La tarjeta **Intermediate English Grades** del home de Course 2 abre `notas.html`. La página reutiliza la grilla madura de Intermediate Course 1 con un endpoint configurable, pero consulta exclusivamente `/api/intermediate2/grades`; no mezcla estudiantes, evaluaciones ni registros de Course 1. El acceso permanece autenticado y no habilita reclamos de identidad adicionales. La grilla inicializa únicamente la evaluación oficial vigente `Midterm Writing Task — 20%`; las prácticas de reading y listening permanecen fuera de ella. El identificador heredado `intermediate2MidtermWritingTask20` se migra a la columna canónica `midtermWritingTask`: cualquier nota o detalle se conserva y la columna duplicada se elimina, de modo que Grades mantiene exactamente cinco evaluaciones oficiales de 20%.
