...
$audioRoot = Join-Path $root "ingles\intermediate\audio\conversation-coach\unit-5"
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
  throw "Missing Conversation Coach script: $scriptPath"
}
...
Write-Output "SUMMARY voice=Matilda-US created=$created skipped=$skipped total=$($items.Count)"
