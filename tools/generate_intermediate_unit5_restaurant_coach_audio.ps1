...
$audioRoot = Join-Path $root "ingles\intermediate\audio\conversation-coach\unit-5-restaurant"
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
  throw "Missing restaurant Conversation Coach script: $scriptPath"
}
...
Write-Output "SUMMARY voice=Ethan-US created=$created skipped=$skipped failed=$failed total=$($items.Count)"
