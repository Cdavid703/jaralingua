...
$audioRoot = Join-Path $root "ingles\intermediate-2\audio\unit-3-technology-functions"
$scriptPath = Join-Path $audioRoot "scripts.md"
$voiceId = "EXAVITQu4vr4xnSDxMaL" # Sarah — clear General American English
...
if ($items.Count -ne 24) { throw "Expected 24 canonical audio clips, found $($items.Count)" }
...
foreach ($item in $items) {
  ...
  $body = @{ text = $item.Text; model_id = "eleven_multilingual_v2"; language_code = "en"; voice_settings = @{ stability = 0.64; similarity_boost = 0.82; style = 0.08; use_speaker_boost = $true } } | ConvertTo-Json -Depth 4
  Invoke-WebRequest -Uri $uri -Method Post -Headers $headers -ContentType "application/json" -Body $body -OutFile $item.Output
  if ((Get-Item -LiteralPath $item.Output).Length -lt 1024) { Remove-Item -LiteralPath $item.Output -Force; throw "Created an invalid MP3: $($item.File)" }
