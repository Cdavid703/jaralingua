  ...
  try {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      if (viewport.label === "laptop") {
        ...
        assert.ok((await page.locator("#readingText").textContent()).includes("Ce qui inquiète"), "Full reset must return to section 1");
      }
      ...
      await page.evaluate(() => {
        ...
        if (finalSummary) {
          ...
          finalSummary.innerHTML = "<h3>Bilan du défi</h3><div><p><span>Fidélité</span><strong>86/100</strong></p><p><span>Progression locale</span><strong>Formative</strong></p></div>";
        }
      });
      ...
      const layout = await page.evaluate(() => {
        const selectors = [
          ...
          ".record-zone", ".mic-calibration", ".metrics", ".actions", ".final-summary"
        ];
        const overflow = [];
      ...
      assert.equal(layout.submitPanelVisible, false, `${viewport.label}: formative 05D must not expose grade submission`);

      const screenshot = path.join(os.tmpdir(), `jaralingua-05d-${viewport.width}x${viewport.height}.png`);
    ...
    const durations = {};
    for (const file of ["section-1.mp3", "section-2.mp3", "section-3.mp3", "section-4.mp3", "n8-05d-medias-desinformation-modele-france.mp3"]) {
    ...
    assert.ok(durations["n8-05d-medias-desinformation-modele-france.mp3"] > 15, "The final model must contain the full paragraph");
    await audioPage.close();

    console.log(`French 8 pronunciation 05D responsive QA passed. Audio durations: ${JSON.stringify(durations)}`);
  } finally {
    await browser.close();
  }
})().catch((error) => {
