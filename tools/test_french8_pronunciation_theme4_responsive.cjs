  ...
  try {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      if (viewport.label === "laptop") {
        ...
        assert.ok((await page.locator("#readingText").textContent()).includes("Lors de l’interview"), "Full reset must return to section 1");
      }
      ...
      const screenshot = path.join(os.tmpdir(), `jaralingua-04d-${viewport.width}x${viewport.height}.png`);
    ...
    const durations = {};
    for (const file of ["section-1.mp3", "section-2.mp3", "section-3.mp3", "section-4.mp3", "n8-04d-discours-rapporte-modele-france.mp3"]) {
    ...
    assert.ok(durations["n8-04d-discours-rapporte-modele-france.mp3"] > 15, "The final model must contain the full paragraph");
    await audioPage.close();

    console.log(`French 8 pronunciation 04D responsive QA passed. Audio durations: ${JSON.stringify(durations)}`);
  } finally {
    await browser.close();
  }
})().catch((error) => {
