...
$audioRoot = Join-Path $root "ingles\intermediate-2\audio\unit-3-explanation"
$scriptPath = Join-Path $audioRoot "scripts.md"
$voiceId = "EXAVITQu4vr4xnSDxMaL"

if (-not (Test-Path -LiteralPath $envFile)) { throw "Missing elevenlabs.local.env" }
if (-not (Test-Path -LiteralPath $scriptPath)) { throw "Missing Unit 3 explanation scripts" }

$settings = @{}
Get-Content -LiteralPath $envFile | ForEach-Object { if ($_ -match '^\s*([^#=]+)=(.*)$') { $settings[$matches[1].Trim()] = $matches[2].Trim() } }
...
if ($items.Count -ne 9) { throw "Expected 9 Unit 3 audio scripts but found $($items.Count)" }
