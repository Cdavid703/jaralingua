...
const pagePath = path.join(root, "ingles", "intermediate", "pair-showcase-unit-5-colombian-food.html");
const practicePath = path.join(root, "ingles", "intermediate", "practice-lab.html");
const jsPath = path.join(root, "assets", "js", "intermediate-unit5-food-showcase.js");
const cssPath = path.join(root, "assets", "css", "intermediate-unit5-food-showcase.css");
const audioRoot = path.join(root, "ingles", "intermediate", "audio", "unit-5-food-showcase");
const imageRoot = path.join(root, "assets", "img", "english-intermediate", "unit-5", "colombian-food-showcase");
...
assert((page.match(/data-audio="audio\/unit-5-food-showcase\/model-/g) || []).length === 12, "The complete model must contain 12 coordinated turns");
assert((page.match(/data-clip="audio\/unit-5-food-showcase\/skill-/g) || []).length === 7, "Seven stage audio models are required");
...
assert(practice.includes("pair-showcase-unit-5-colombian-food.html") && practice.includes("Unit 5 - 13 activities"), "Practice Lab Unit 5 integration is incomplete");
assert(practice.includes("Unit 6 - 9 activities"), "Practice Lab Unit 6 count must exclude the food showcase");
...
const audioFiles = Array.from(new Set(Array.from(page.matchAll(/audio\/unit-5-food-showcase\/([^"]+\.mp3)/g), (match) => match[1])));
...
console.log("PASS Unit 5 Colombian Food Cultural Showcase: joint pedagogy, 19 professional audios, two visuals, organizer, checklist, and responsive contracts");
