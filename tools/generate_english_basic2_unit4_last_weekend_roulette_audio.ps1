...
$audioRoot = Join-Path $root "ingles\basico-2\audio\unit4\last-weekend-roulette"
$scriptPath = Join-Path $audioRoot "teacher-model-audio-script.md"
...
for ($index = 0; $index -lt $content.Count; $index++) {
  if ($content[$index] -match '^File:\s+`([^`]+)`') {
    ...
    [void]$items.Add([pscustomobject]@{ Text = ($textLines -join " "); Output = Join-Path $audioRoot $fileName })
  }
}
...
foreach ($item in $items) {
  if ((Test-Path -LiteralPath $item.Output) -and -not $Overwrite) { Write-Output ("SKIPPED " + $item.Output); continue }
  New-Item -ItemType Directory -Path (Split-Path -Parent $item.Output) -Force | Out-Null
  $body = @{ text = $item.Text; model_id = "eleven_multilingual_v2"; language_code = "en"; voice_settings = @{ stability = 0.62; similarity_boost = 0.82; style = 0.12; use_speaker_boost = $true } } | ConvertTo-Json -Depth 4
  Invoke-WebRequest -Uri $uri -Method Post -Headers $headers -ContentType "application/json" -Body $body -OutFile $item.Output
  Write-Output ("CREATED " + $item.Output)
}
