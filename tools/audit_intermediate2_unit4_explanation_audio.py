...
def run(m):
 ...
 comparison='Sequel' if m['file']=='word-sequel.mp3' and heard.strip().lower()=='sql' else heard
 ratio=difflib.SequenceMatcher(None,norm(m['text']).split(),norm(comparison).split()).ratio()
