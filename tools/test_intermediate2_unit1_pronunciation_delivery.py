...
os.environ.setdefault("JARALINGUA_LOCAL_AUTH_SECRET", "intermediate2-pronunciation-test-secret")

from server import progress_api


class Intermediate2Unit1PronunciationDeliveryTests(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.TemporaryDirectory()
        progress_api.INTERMEDIATE2_UNIT1_PRONUNCIATION_SUBMISSIONS_PATH = os.path.join(
        ...
        self.payload = {
            "clientSubmissionId": "delivery-test-0001",
            "details": {
                "overall": 88,
                "accuracy": 90,
                "completeness": 92,
                "fluency": 78,
                "wpm": 112,
                "transcript": progress_api.INTERMEDIATE2_UNIT1_PRONUNCIATION_REFERENCE,
                "referenceText": progress_api.INTERMEDIATE2_UNIT1_PRONUNCIATION_REFERENCE,
                "missedWords": ["recently"],
                "stageLabel": "Final challenge",
                "audioDataUrl": "data:audio/webm;base64," + base64.b64encode(b"webm-test-audio").decode("ascii"),
            },
        }
    ...
    def test_delivery_is_idempotent_and_never_projects_to_grades(self):
        status, first = progress_api.submit_intermediate2_unit1_pronunciation(self.profile, self.payload)
        ...
        status, replay = progress_api.submit_intermediate2_unit1_pronunciation(self.profile, self.payload)
        self.assertEqual(status, 200)
        self.assertTrue(replay["idempotentReplay"])
        self.assertEqual(first["receiptId"], replay["receiptId"])

        with open(progress_api.INTERMEDIATE2_UNIT1_PRONUNCIATION_SUBMISSIONS_PATH, encoding="utf-8") as handle:
    ...
    def test_modified_reference_is_rejected_before_audio_storage(self):
        ...
        status, response = progress_api.submit_intermediate2_unit1_pronunciation(self.profile, self.payload)
