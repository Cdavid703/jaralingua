...
assert(!page.includes('id="inlineWordCounter"'), 'Duplicate inline word counter below the writing area must stay removed.');
assert(!page.includes('id="inlineSubmitHint"'), 'Duplicate inline submit hint below the writing area must stay removed.');
assert(/<textarea id="emailBody"[\s\S]*placeholder="Write your complete magazine post here\."/.test(page), 'Writable exam textarea needs a clear placeholder.');
assert(/<button class="writing-button primary full" type="submit" id="submitButton">/.test(page), 'Submit button must not be initially disabled in the exam markup.');
...
assert(js.includes('unlockWritingFields'), 'Writing fields must be explicitly unlocked when the exam opens.');
assert(js.includes('The writing area is open'), 'Students need a clear message that the writing area is ready.');
assert(!js.includes('inlineWordCounter'), 'JS must not keep references to the removed inline word counter.');
assert(!js.includes('inlineSubmitHint'), 'JS must not keep references to the removed inline submit hint.');
