...
const BASE_URL = process.env.JARALINGUA_TEST_URL || "http://127.0.0.1:8022/ingles/intermediate/game-unit-4-family-impostor.html";
...
const card = {
  ...
  familyContext: "A student is excited about a future study-abroad plan.",
...
function roomState(status, round = 1) {
  return {
    ...
    round,
    deck: "unit2",
    deckLabel: "Unit 2: wishes, hopes, dreams, and experiences",
...
async function preparePage(browser, role, viewport) {
  ...
  await context.addInitScript(({ role, user }) => {
    ...
    if (role === "teacher") {
      ...
      localStorage.setItem("english-intermediate-unit4-impostor-live-state", JSON.stringify({
        roomCode: "TST4",
...
async function installTeacherApi(page) {
  let status = "waiting";
  let round = 1;
  await page.route("**/api/intermediate/unit4-impostor**", async route => {
    ...
    if (body.action === "create") {
      status = "waiting";
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true, roomCode: "TST4", teacherToken: "teacher-room-token", state: teacherPayload(status, round) }) });
      return;
    }
    ...
    if (body.action === "reset") { status = "waiting"; round += 1; players.forEach(player => { player.hasVoted = false; player.voteSuspectId = ""; }); }
...
async function assertResponsive(page, label) {
  ...
  await page.screenshot({ path: path.join(os.tmpdir(), "jaralingua-impostor-" + label + ".png"), fullPage: true });
}
...
async function testTeacherFlow(browser) {
  ...
  await page.waitForFunction(() => document.querySelector("#roomCodeDisplay")?.textContent.trim() === "TST4");
  assert.equal((await page.locator("#roomCodeDisplay").innerText()).trim(), "TST4");
  ...
  assert.match(await page.locator("#selectedDeckNote").innerText(), /Unit 2 vocabulary/);
  await page.locator("#distributeBtn").click();
  assert.match(await waitForStatus(page, "Roles distributed"), /Roles distributed with Unit 2 vocabulary/);
...
async function testStudentVoteAndResponsive(browser) {
  for (const viewport of viewports) {
    ...
    assert.match(await page.locator("#studentConnectionText").innerText(), /Connected as Alex.*Room TST4/);
...
(async () => {
  ...
  try {
    ...
    console.log("PASS Vocabulary Impostor UI: teacher feedback, vote update, images, and responsive layouts");
  } finally {
    await browser.close();
  }
})().catch(error => {
