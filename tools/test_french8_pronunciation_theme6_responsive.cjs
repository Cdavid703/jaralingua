  ...
  try {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      if (viewport.label === "laptop") {
        ...
        assert.ok((await page.locator("#readingText").textContent()).includes("Étant donné"), "Full reset must return to section 1");
      }
      ...
      assert.equal(layout.submitPanelVisible, false, `${viewport.label}: formative 06D must not expose grade submission`);

      const screenshot = path.join(os.tmpdir(), `jaralingua-06d-${viewport.width}x${viewport.height}.png`);
    ...
    const durations = {};
    for (const file of ["section-1.mp3", "section-2.mp3", "section-3.mp3", "section-4.mp3", "n8-06d-ia-ethique-modele-france.mp3"]) {
    ...
    assert.ok(durations["n8-06d-ia-ethique-modele-france.mp3"] > 15, "The final model must contain the full paragraph");
    await audioPage.close();

    console.log(`French 8 pronunciation 06D responsive QA passed. Audio durations: ${JSON.stringify(durations)}`);
  } finally {
    await browser.close();
  }
})().catch((error) => {
