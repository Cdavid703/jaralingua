...
assert.equal(await page.locator('.tg-card').count(),18);
assert.equal(await page.locator('.tg-card svg image').count(),18,'Every card must contain its picture');
assert.equal(await page.locator('.tg-front[aria-hidden=false]').count(),0);
assert.deepEqual(await page.locator('.tg-back').allTextContents(),Array.from({length:18},(_,i)=>String(i+1)));
assert.deepEqual(await page.locator('.mm-card-number').allTextContents(),Array.from({length:18},(_,i)=>String(i+1)));
...
assert.equal(await page.locator('#memoryScore1').innerText(),'9');
assert.equal(await page.locator('#memory-complete').isVisible(),true);
assert.equal(await page.locator('#memory-history li').count(),9);
for(const width of [320,390,768,1024,1440,1920]){
 ...
 const columns=await page.locator('#memory-board').evaluate(e=>getComputedStyle(e).gridTemplateColumns.split(' ').length);
 assert.equal((await page.locator('.tg-card').count())%columns,0,'No incomplete rows at '+width);
...
for(const theme of ['music','mixed','film','genres']){
 ...
 const count=['music','mixed'].includes(theme)?24:18;
