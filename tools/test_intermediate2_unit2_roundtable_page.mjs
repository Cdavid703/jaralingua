...
assert.match(page, /id="dilemmaCase"/, "Choosing a dilemma must reveal a detailed case sheet.");
assert.match(page, /var caseStudies =/, "Every dilemma must have its own case-study data.");
for (const title of ["Course or job", "Move or stay", "Save or create", "Second chance"]) assert.match(page, new RegExp(`"${title}": \\{`), `${title} needs its own expanded case.`);
assert.match(page, /One question for each role/);
assert.match(page, /Consequence to weigh/);
assert.match(page, /Say this together at the end/);
