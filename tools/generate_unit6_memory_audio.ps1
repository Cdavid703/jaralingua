...
$audioRoot = Join-Path $root "ingles\basico\audio\unit6"
$scriptPath = Join-Path $audioRoot "cards\unit-6-neighborhood-memory-card-scripts.md"
...
for ($index = 0; $index -lt $content.Count; $index++) {
  if ($content[$index] -match '^File:\s+`([^`]+)`') {
    ...
    [void]$items.Add([pscustomobject]@{
      Text = ($textLines -join ' ')
      Output = Join-Path $audioRoot ("cards\" + $fileName)
    })
  }
}
