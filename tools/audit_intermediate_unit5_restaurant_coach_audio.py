"""Audit Unit 5 Cedar & Stone Conversation Coach audio with ElevenLabs Scribe."""
...
AUDIO_DIR = ROOT / "ingles" / "intermediate" / "audio" / "conversation-coach" / "unit-5-restaurant"
...
def scribe(audio_path: Path, api_key: str) -> str:
    boundary = "----JaraLinguaCedarStoneBoundary"
...
def main() -> int:
    ...
    if len(items) != 30:
        print(f"Expected 30 scripts, found {len(items)}", file=sys.stderr)
        return 2
    load_env()
