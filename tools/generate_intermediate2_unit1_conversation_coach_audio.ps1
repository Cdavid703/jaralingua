...
$audioRoot = Join-Path $root "ingles\intermediate-2\audio\conversation-coach\unit-1-coffee-with-gabriel"
$scriptPath = Join-Path $audioRoot "scripts.md"

if (-not (Test-Path -LiteralPath $envFile)) { throw "Missing elevenlabs.local.env" }
if (-not (Test-Path -LiteralPath $scriptPath)) { throw "Missing conversation script: $scriptPath" }
...
foreach ($item in $items) {
  ...
  $body = @{
    ...
    language_code = "en"
    voice_settings = @{ stability = 0.61; similarity_boost = 0.82; style = 0.18; use_speaker_boost = $true }
