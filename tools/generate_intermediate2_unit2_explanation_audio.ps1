...
$audioRoot = Join-Path $root "ingles\intermediate-2\audio\unit-2-explanation"
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
  throw "Missing Unit 2 explanation script: $scriptPath"
}
...
if ($items.Count -ne 14) {
  throw "Expected 14 Unit 2 audio scripts but found $($items.Count)"
}
